...
def rootsarr(Nq):#function to store the roots for each q value
    roots_arr = np.zeros((Nq, Nr, 3)) # Initialize an array to store the roots for each q value
...
def psiV_func(E, Ev, grad):#function to calculate valence band wavefunction
    const = np.sqrt(p*me/2 + 1j)*(hbar)
    return (1/(E - Ev))*const*grad
...
def derivative_arr(i, arr):#bespoke derivative calculator with the correct Bloch boundary conditions
    ...
    del_arr[-1] = (arr[-2] - np.exp(2j*q[i]*d)*arr[0])/ (2*dz)
...
def Total_Bloch(v):
    Bloch_arr = np.zeros((Nq, 2, 300)) #initialize array to hold the Bloch wavefunctions for all q
...
def BlochPhase(v):
    arr = Total_Bloch(v)
    Xarr = np.zeros((Nq, 2))#initialise array to store Xv(q)
    for i in range(Nq):
