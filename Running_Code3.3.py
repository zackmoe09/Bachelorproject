...
def BlochPhase2(arr):
    ...
    for i in range(Nq):
        for j in range(2):
            ...
            if i==0:
                Darr[j,:] = (arr[1,j,:]*np.exp(-1j*q[1]*z[:])
                           -arr[-1,j,:]*np.exp(-1j*(q[-1]-2*np.pi/d)*z[:]))*Nq*d/2/np.pi
            elif i==Nq-1:
...
plotter2(3, 0, colors[0], 0, ax, factors)
plotter2(3, 1, colors[1], 0, ax, factors)
plotter2(3, 2, colors[2], 0, ax, factors)
plotter2(3, 3, colors[3], 0, ax, factors)
plotter2(3, 4, colors[4], 0, ax, factors)
plotter2(3, 5, colors[5], 0, ax, factors)
...
def expectationvaluez(n, v):
    FullBloch = fullblochtest(n, v)
    Bloch =Total_Bloch(v)
    #zmax = np.linspace(-n*d, (n+1)*d, (2*n+1)*Nz, endpoint=False)
    phase = BlochPhase2(Bloch)
    #arr2 = arr * np.expand_dims(np.exp(1j)**(np.outer(q, z)), axis=1)
    ...
    norm = sum((abs(wannier_func[0])**2+abs(wannier_func[1])**2)*dz)
    
    return norm

for i in range(Nr):
    print(expectationvaluez(3, i))

#%%
def fullblochtest(n, v):
    arr = Total_Bloch(v)
    zmax = np.linspace(-n*d, (n+1)*d, (2*n+1)*Nz, endpoint=False) #NEW
...
#%%
plt.plot(zmax, wannierSimp(3, 0)[0,0,:])
plt.plot(zmax, fullblochtest(3, 0)[0,0,:])
...
def blochphase3(arr):
    ...
    return np.roll(phase, -450)


 
    
#plt.plot(q, BlochPhase2(Total_Bloch(0)))
plt.plot(q, blochphase3(Total_Bloch(0)))

#%%



delarr = (np.roll(arr, -1, axis=0)*np.exp(-1j*np.outer(np.roll(q, -1), z))
          - np.roll(arr, 1, axis=0)*np.exp(-1j*np.outer(np.roll(q, 1), z)))*Nq*d/(4*np.pi)
delarr[0,:,:] = ((arr[1,:,:]*np.exp(-1j*q[1]*z))
                      - (arr[-1,:,:]* np.exp(-1j*(q[-1] - (2*np.pi/d))*z)))*Nq*d/2/np.pi
delarr[-1,:,:] = (arr[0,:,:]*np.exp(-1j*(q[0]+(2*np.pi/d))*z[:])
                      -arr[-2,:,:]*np.exp(-1j*q[-2]*z[:]))*Nq*d/2/np.pi
