...
def BlochPhase2(arr):
    ...
    for i in range(Nq):
        Harr[0,:] = arr[i,0,:]*np.exp(-1j*q[i]*z[:])
        Harr[1,:] = arr[i,1,:]*np.exp(-1j*q[i]*z[:])
        Xarr[i]=1j*dz*sum(np.conjugate(Harr[0,:])*derivative_arr(i, Harr[0,:])
                         +np.conjugate(Harr[1,:])*derivative_arr(i, Harr[1,:]))
...
def plotter2(n, v):
    ...
    for i in range(Nq):
            fullarr[i,:,:] = fullarr[i,:,:]*np.exp(1j*phase[i])
            
    wannier_func = (np.sum(fullarr, axis=0))/Nq
    
    plt.plot(zmax, np.real(abs(wannier_func[0])**2))
    plt.show
    #plt.plot(zmax, np.imag(wannier_func[1]))
...
def wannier2(v):
    ...
    for i in range(Nq):
        for j in range(2):
            arr[i,j,:] = arr[i,j,:]*np.exp(1j*phase[i,j])
            
    wannier_func = (np.sum(arr, axis=0))/Nq
