...
thlist = np.array([3.3, 6.43, 1.9, 14.7, 2.4, 8.33]) # thickness of the layers
Ecn = np.array([0.303, 0, 0.303, 0, 0.303, 0])# conduction band offset
mcn = np.array([0.0919, 0.067, 0.0919, 0.067, 0.0919, 0.067])# eff mass, in me
p = 22.67# Eg/meff
d = sum(thlist)# 1e-9 # thickness of module
Nq = 900 #Number of q values
Ev = Ecn - (p*mcn)#Valence band offset
Nr = 9#Number of bands
Nz = 300
...
ax.set_xlim(-d, 2*d)
plt.plot(zmax, qcl, 'black', linewidth = '0.5')   
...
plotter2(3, 6, colors[0], 0, ax, factors[6])
plotter2(3, 7, colors[2], 0, ax, factors[0])
plotter2(3, 8, colors[3], 0, ax, factors[0])
